...
pub mod native {
  ...
  use irondash_dart_ffi::{raw, DartPort, DartValue, NativePort};
    ...
    // Shift the PID left to serve as a completely unique high-bit prefix per instance,
    // while keeping it safely inside standard i64 range limits.
    ...
    pub(crate) extern "C" fn register_isolate(port: i64, _isolate_id: *mut c_void) -> i64 {
        ...
        warn!("Process-Isolated Id generated by message channel: {:?}", isolate_id);
